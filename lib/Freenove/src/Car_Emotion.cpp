...
byte cat_left[][8] =
{
  0x00, 0x00, 0x30, 0x30, 0x00, 0x09, 0x06, 0x00,
  0x00, 0x00, 0x30, 0x30, 0x00, 0x00, 0x09, 0x06,
};

//wheel-right
byte cat_right[][8] =
{
  0x00, 0x00, 0x0C, 0x0C, 0x00, 0x90, 0x60, 0x00,
  0x00, 0x00, 0x0C, 0x0C, 0x00, 0x00, 0x90, 0x60,
};
...
{
  int count = sizeof(cat_left) / sizeof(cat_left[0]);
    emotion_time_next = millis();
    if (emotion_time_next - emotion_time_now >= delay_ms)
    {
      emotion_time_now = emotion_time_next;
      matrix.showStaticArray(cat_left[emotion_count], cat_right[emotion_count]);
      emotion_count++;
      if (emotion_count >= count)
        emotion_count = 0;
    }
...
{
  if (mode == -1) { // -1 means handled manually via websocket
    if (emotion_mode == -1)
      return;
    emotion_mode = mode;
    clearEmtions();
